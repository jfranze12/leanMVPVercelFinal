...
function getPrisma() {
  ...
  if (!globalForPrisma.__leanMvpPrisma) {
    globalForPrisma.__leanMvpPrisma = new PrismaClient()
  }
  return globalForPrisma.__leanMvpPrisma
}
...
async function q(strings, ...values) {
  ...
  const isRead = command === 'SELECT' || command === 'WITH'
